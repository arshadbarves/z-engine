...
use super::local_commands::{run_hook, run_shell_passthrough};
use super::mcp_setup;
        ...
        match command {
            Command::SubmitMessage {
                ...
                if !titled {
                    ...
                    let titling = tokio::spawn(async move {
                    ...
                    // The titler runs beside the turn rather than inside
                    // it, so the turn would otherwise end before its
                    // request reached the tape.
                ...
                if let Some(run) = &ctx.run_recorder {
                    ...
                    // A run that could not record itself has no business
                    // reporting success: the cassette it left behind is
                    // short, and a short cassette replays as a run that
                    // never happened.
                    if let Some(fault) = run.fault() {
                        outcome = TurnOutcome::Failed(recording_failed(&fault));
                    }
                    run.record_turn(&outcome.label());
                    // The metrics line can itself be the entry that is
                    // lost; the tape is condemned either way, but the
                    // outcome has to say so too.
                    if let Some(fault) = run.fault() {
                        outcome = TurnOutcome::Failed(recording_failed(&fault));
                    }
                }
...
/// How a run reports that it failed to record itself. The wording is
/// deliberately about reproducibility rather than about the turn: the
/// work may well have happened, but the evidence of it did not.
fn recording_failed(fault: &crate::replay::RecordingFault) -> String {
    format!("this run could not record itself, so it is not reproducible: {fault}")
}
